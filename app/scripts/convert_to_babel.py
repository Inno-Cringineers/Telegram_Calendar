#!/usr/bin/env python3
"""Script to convert existing strings to Babel format.

This script reads the old strings dictionary and creates .po files for Babel.
"""

import json
from pathlib import Path

# Old strings structure (copied from strings.py)
OLD_STRINGS = {
    "en": {
...
def create_po_file(lang: str, strings: dict[str, str], output_path: Path) -> None:
    """Create a .po file for a language.

    Args:
        lang: Language code.
        strings: Dictionary of translation strings.
        output_path: Path to output .po file.
    """
    po_content = f'''# Translation file for {lang.upper()}
# Copyright (C) 2024
msgid ""
msgstr ""
"Language: {lang}\\n"
...
'''
    for key, value in sorted(strings.items()):
        ...
        msgstr = value.replace("\\", "\\\\").replace('"', '\\"').replace("\n", "\\n")
...
def main() -> None:
    """Main function to convert strings to Babel format."""
    ...
    for lang in ["en", "ru"]:
        ...
        create_po_file(lang, OLD_STRINGS[lang], po_file)
