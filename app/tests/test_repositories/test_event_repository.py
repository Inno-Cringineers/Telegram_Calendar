...
from repositories.schemas import EventCreateSchema, EventDurationFilter, EventFilter, EventResponse, EventUpdateSchema
...
async def test_get_by_id_returns_event(
    ...
    assert result == EventResponse.from_model(sample_event)
...
async def test_create_creates_event(event_repository: EventRepository, mock_session: AsyncMock) -> None:
    ...
    result = await event_repository.create_one(create_data[0])

    assert result.user_id == 12345
    assert result.uid == "test-uid-1"
    assert result.calendar_id == 1
    assert result.date_start == start_time
    assert result.date_end == end_time
    assert result.title == "New Event"
    assert result.description == "New event description"
...
async def test_create_creates_multiple_events(event_repository: EventRepository, mock_session: AsyncMock) -> None:
    ...
    result = await event_repository.create_many(create_data)
...
async def test_create_creates_event_with_all_fields(event_repository: EventRepository, mock_session: AsyncMock) -> None:
    ...
    result = await event_repository.create_one(create_data)
...
async def test_update_updates_existing_event(
    ...
    result = await event_repository.update_by_id(1, update_data)
...
async def test_update_raises_error_when_event_not_found(
    ...
    with pytest.raises(EventNotFoundError) as exc_info:
        await event_repository.update_by_id(999, update_data)
...
async def test_delete_deletes_event(
    ...
    await event_repository.delete_by_id(1)
...
async def test_delete_raises_error_when_event_not_found(
    ...
    with pytest.raises(EventNotFoundError) as exc_info:
        await event_repository.delete_by_id(999)
...
async def test_find_returns_all_events_without_filters(
    ...
    filter_data = EventFilter()

    result = await event_repository.find(filter_data)

    assert len(result) == 2
    assert result == [EventResponse.from_model(event) for event in events]
...
async def test_find_filters_by_user_id(event_repository: EventRepository, mock_session: AsyncMock) -> None:
    ...
    mock_result.scalars.return_value.all.return_value = events
    mock_session.execute.return_value = mock_result
    filter_data = EventFilter(user_id=12345)

    result = await event_repository.find(filter_data)

    assert len(result) == 1
    assert result[0] == EventResponse.from_model(events[0])
    mock_session.execute.assert_called_once()
...
async def test_find_filters_by_uid(event_repository: EventRepository, mock_session: AsyncMock) -> None:
    ...
    mock_result.scalars.return_value.all.return_value = events
    mock_session.execute.return_value = mock_result
    filter_data = EventFilter(uid="event-123")
...
async def test_find_filters_by_calendar_id(event_repository: EventRepository, mock_session: AsyncMock) -> None:
    ...
    mock_result.scalars.return_value.all.return_value = events
    mock_session.execute.return_value = mock_result
    filter_data = EventFilter(calendar_id=1)
...
@pytest.mark.asyncio
async def test_find_by_duration_filters_by_duration_from(
    event_repository: EventRepository, mock_session: AsyncMock
) -> None:
    """Test that find_by_duration filters events by duration_from."""
    start_time = datetime(2025, 1, 1, 10, 0, tzinfo=UTC)
    end_time = datetime(2025, 1, 1, 11, 0, tzinfo=UTC)
    events = [
        Event(
    ...
    mock_result.scalars.return_value.all.return_value = events
    mock_session.execute.return_value = mock_result
    mock_session.get.return_value = events[0]
    filter_data = EventDurationFilter(
        user_id=12345,
        duration_from=datetime(2025, 1, 1, 0, 0, tzinfo=UTC),
        duration_to=datetime(2025, 1, 1, 23, 59, tzinfo=UTC),
    )

    result = await event_repository.find_by_duration(filter_data)
...
@pytest.mark.asyncio
async def test_find_by_duration_applies_multiple_filters(
    event_repository: EventRepository, mock_session: AsyncMock
) -> None:
    """Test that find_by_duration can apply multiple filters simultaneously."""
    start_time = datetime(2025, 1, 1, 10, 0, tzinfo=UTC)
    end_time = datetime(2025, 1, 1, 11, 0, tzinfo=UTC)
    events = [
        Event(
...
async def test_find_returns_empty_list_when_no_matches(
    ...
    mock_session.execute.return_value = mock_result
    filter_data = EventFilter(user_id=99999)
